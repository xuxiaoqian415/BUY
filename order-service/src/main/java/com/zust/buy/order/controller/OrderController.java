...
//        resultMap.put("paySign",getSign(resultMap));
    ...
    public Map<String, Object> list(Integer type, Integer page, Integer pageSize) {
        ...
        if (type == 0) {
            orderPage = orderService.page(orderPage, new QueryWrapper<Order>().orderByDesc("id"));
        } else {
            orderPage = orderService.page(orderPage, new QueryWrapper<Order>().orderByDesc("id").eq("status", type));
        }
